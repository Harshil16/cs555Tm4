...
class GedList:
    def __init__(self, filename):
        self.tagchecks = {"INDI": 0, "NAME": 1, "SEX": 1, "BIRT": 1, "DEAT": 1, "FAMC": 1, "FAMS": 1, "FAM": 0,
                          "MARR": 1, "HUSB": 1, "WIFE": 1, "CHIL": 1, "DIV": 1, "DATE": 2, "TRLR": 0, "NOTE": 0}
        self.list = {}
        ...
        for id, item in self.list.iteritems():
            if re.search("@I.+", id):
                ...
                if item.fams is not None and item.famc is not None:
                    spouseId = self.list[item.fams].wife if self.list[item.fams].husb == id else self.list[
                        item.fams].husb
                    ...
                    if parentId1 == spouseId:
                        testFail = True
                        print item.firstname + " " + item.lastname + " is married to their parent " + self.list[
                            parentId1].firstname + " " + self.list[parentId1].lastname + " -- Failed"
                    elif parentId2 == spouseId:
                        testFail = True
                        print item.firstname + " " + item.lastname + " is married to their parent " + self.list[
                            parentId2].firstname + " " + self.list[parentId2].lastname + " -- Failed"
            ...
            if re.search("@I.+", id):
                ...
                if item.fams is not None:
                    ...
                    if self.list[spouseId].getAge() < 18:
                        testFail = True
                        print item.firstname + " " + item.lastname + " is married to a minor " + self.list[
                            spouseId].firstname + " " + self.list[spouseId].lastname + " -- Failed"
        ...
        for id, item in self.list.iteritems():
            if "@I" in id:
                ...
                if item.marr is not None:
                    ...
                    timelinelist[
                        marrday] = husband.firstname + " " + husband.lastname + " and " + wife.firstname + " " + wife.lastname + " were married on " + str(
                        marrday)
                if item.div is not None:
                    divday = datetime.datetime.strptime(item.div, "%d %b %Y").date()
                    timelinelist[
                        divday] = husband.firstname + " " + husband.lastname + " and " + wife.firstname + " " + wife.lastname + " got divorced on " + str(
                        divday)
    ...
    def deathMarriageCheck(self):
        for id, item in self.list.iteritems():
            if re.search("MARR", id):
                testFail = False

                marrDate = datetime.datetime.strptime(item.marr, "%d %b %Y").date()
                if self.list[item.husb].deat is None and self.list[item.wife].deat is None:
                    continue
                ...
                if husbDeathDate > marrDate or wifeDeathDate > marrDate:
                    testFail = True
                    print " there was a death before marriage -- Failed"

                if testFail == False:
                    print " death and marriage dates look normal -- Passed"
    ...
    def marriageBirthCheck(self):
        for id, item in self.list.iteritems():
            if re.search("MARR", id):
                ...
                marrDate = datetime.datetime.strptime(item.marr, "%d %b %Y").date()
                if self.list[item.husb].birt is None and self.list[item.wife].birt is None:
                    continue
                ...
                if husbBirthDate > marrDate or wifeBirthDate > marrDate:
                    testFail = True
                    print " there was a birth after marriage -- Failed"

                if testFail == False:
                    print " birth and marriage dates look normal -- Passed"


# and now for the main

g = GedList("gedcoms/us2.ged")
#g.printList()
